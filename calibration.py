...
@click.option(
    ...
    type=bool,
    default=False,
    help="Do not perform any calibration, just run the code to calculate the radius of gyration and mobility index. "
    "Make sure to pass an absolute value to tick-averages-file in that case",
)
@click.pass_context
def behavior(
    ctx, mobility_index_file, tick_averages_file, sliding_window_size, test_gyration
):
...
@click.option(
    "--scale-factor",
    help="Scale the actual recorded case count data with this value before comparing, to account for uncertainty in testing",
    default=30,
)
@click.pass_context
def disease(
