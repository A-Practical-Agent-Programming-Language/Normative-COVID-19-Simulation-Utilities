...
@click.group()
@click.option('--county-configuration', '-c', type=click.Path(exists=True),
              help='Specify the TOML file containing the county configuration for both the agent model and the calibration process',
              required=True)
@click.option('--behavior-model', '-b', type=click.Path(exists=True),
...
@click.option('--output-dir', '-o', type=str, default="output",
...
@click.command(name="behavior", help="Start the behavior model calibration with the disease model fixed")
...
@click.option("--tick-averages-file", type=str, default="tickaverages.csv",
              help="Specify the name (not the directory) the tickaverages file generated by the behavior model will get")
@click.pass_context
def behavior(ctx, mobility_index_file, tick_averages_file):
	...
	bc = BehaviorCalibration(gyration=gyration, tick_averages_file=tick_averages_file, **ctx.obj["args"])
	initial_simplex = [
		[0.5, 0.5],
		[0.5, 0.75],
		[0.75, 0.5]
	]
	calibrate(bc.calibrate, initial_simplex, [0.5, 0.5])


@click.command(name="disease",
...
def disease(ctx, mode_liberal, mode_conservative):
	...
	calibrate(dc.calibrate, [[1], [0.001]], [1 / 24])


def calibrate(fitness_function, initial_simplex, x0):
	options = {'xatol': 0.1, 'disp': True, "initial_simplex": initial_simplex}
	minimize(fitness_function, x0=x0, method="nelder-mead", options=options)
