...
@click.group()
@click.option(
	'--county-configuration', '-c', type=click.Path(exists=True, dir_okay=True, file_okay=True, resolve_path=True),
	help='Specify the TOML file containing the county configuration for both the agent model and the calibration process',
	required=True
)
@click.option(
	'--behavior-model', '-b', type=click.Path(exists=True, dir_okay=True, file_okay=True, resolve_path=True),
...
@click.option(
	'-Xmx', type=str, default="64g",
	help=
	"Specify the maximum heap size following Java argument formatting (see Java man pages for help, Java argument is -Xmx"
)
@click.option(
	'-Xms', type=str, default="55g",
	help="Specify the initial heap size following Java argument formatting (see Java man pages for help, Java argument is -Xms"
)
@click.option(
	'--output-dir', '-o', type=click.Path(dir_okay=True, file_okay=True, resolve_path=True), default="output/agent-runs",
...
@click.option(
	'--mobility-index-file', '-m', type=click.Path(exists=True),
...
@click.option(
	"--tick-averages-file", type=str, default="tick-averages.csv",
...
@click.option(
	...
	default=7,
	help=
	"Specify the size of the sliding window with which the mobility index will be smoothed when calculating the "
...
def disease(ctx, mode_liberal, mode_conservative, fatigue, fatigue_start, case_data_file, scale_factor):
	...
	args["epicurve_rmse"] = Epicurve_RMSE(ctx.obj['counties'], case_data_file)
...
@start.command(
	name="simplerepeat",
	help=
	"Run the experiment n_runs number of times with a fixed disease model and behavior parameters, "
	"and exit after finishing"
)
@click.option(
	'--mode-liberal', '-ml', default=0.2,
	help="Specify the mode of the government trust factor for the liberal voting agents"
)
@click.option(
	'--mode-conservative', '-mc', default=0.2,
	help="Specify the mode of the government trust factor for the liberal voting agents"
)
@click.option("--fatigue", help="The fatigue factor with which the agents' trust attitude will decrease each day")
@click.option("--fatigue-start", help="The start time step for decreasing the agents' trust attitude with fatigue")
...
@start.command(name="sensitivity", help="Perform simulation with the most extreme parameters to test sensitivity to reasoning")
@click.option(
	'--mobility-index-file', '-m', type=click.Path(exists=True),
	help="Specify the location of the file containing the mobility index for each (relevant) county",
	default=os.path.join("external", "va_county_mobility_index.csv")
