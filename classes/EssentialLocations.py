...
class EssentialDesignationExtractor(object):
    ...
    def from_synthetic_population_directory(self, synthetic_population_location: str):
        ...
        try:
            return self.__extract_for_county(
                os.path.join(
                    self.synthetic_population_location, "location_designation.csv"
                )
            )
        except PermissionError:
            fname = (
                "location_designation_"
                + os.path.split(self.synthetic_population_location.rstrip(os.path.sep))[
                    -1
                ]
                + ".csv"
            )
            os.makedirs(
                os.path.join(".persistent", "location_designations"), exist_ok=True
            )
            return self.__extract_for_county(
                os.path.join(".persistent", "location_designations", fname)
            )

    def from_county(self, county: dict):
        self.synth_pop_files = {
            "activities": county["locations"]
            if "locations" in county
            else county["activities"],
        ...
        try:
            return self.__extract_for_county(
                self.synth_pop_files["household"][0].replace(
                    "household", "essential_location_designation"
                )
            )
        except PermissionError:
            fname = os.path.split(
                self.synth_pop_files["household"][0].rstrip(os.path.sep)
            )[-1].replace("household", "essential_location_designation")
            os.makedirs(
                os.path.join(".persistent", "location_designations"), exist_ok=True
            )
            return self.__extract_for_county(
    ...
    def __extract_for_county(self, output_file):
        (
            ...
            self.num_essential_agents,
        ) = self.load_essential_workers()
    ...
    def find_files(self) -> Dict[str, List[str]]:
        ...
        """
        synth_pop_files = {"person": list(), "activities": list(), "household": list()}

        for f in os.listdir(self.synthetic_population_location):
            if f.endswith(".csv"):
                if "location" in f and "designation" not in f or "activit" in f:
                    synth_pop_files["activities"].append(
                        os.path.join(self.synthetic_population_location, f)
                    )
                elif "household" in f:
                    synth_pop_files["household"].append(
                        os.path.join(self.synthetic_population_location, f)
                    )
                elif "person" in f:
                    synth_pop_files["person"].append(
                        os.path.join(self.synthetic_population_location, f)
                    )
        ...
        if "location" in "".join(synth_pop_files["activities"]):
            synth_pop_files["activities"] = [
                act_file
                for act_file in synth_pop_files["activities"]
                if "location" in act_file
            ]
        ...
        for f in self.synth_pop_files["household"]:
            with open(f, "r") as homes_in:
                ...
                for line in homes_in:
                    home = line[:-1].split(",")
                    homes[home[residence_id]] = (
                        home[business_index] == "1" or homes[home[residence_id]]
                    )
    ...
    def _assign_essential_locations(self) -> Dict[str, Set[str]]:
        ...
        for f in self.synth_pop_files["activities"]:
            with open(f, "r") as locations_in:
                ...
                for location in locations_in:
                    ...
                    if (
                        data[pid_index] in self.essential_workers
                        and data[type_index] == "2"
                    ):
        ...
        if len(double_work_locations):
            print(
                "WARNING: The following essential worker PID's have a different number of work locations than 1:"
            )
    ...
    def write_locations_file(self, output_file):
        if os.path.exists(output_file):
            print(
                output_file,
                "already exists. Please remove this file to create it again. Reusing for now",
            )
            return output_file
    ...
    def add_residences_to_location_designations(
        self,
    ) -> Dict[str, str]:
        ...
        for home in self.home_locations:
            if home in assigned_locations and not self.home_locations[home]:
                print(
                    f"WARNING: Home location {home} already assigned as {assigned_locations[home]}"
                )
    ...
    def mark_government_designation_with_highest_num_unique_visits_as_dmv(self):
        ...
        for lid in sorted(
            self.essential_locations.keys(), key=lambda x: unique[x], reverse=True
        ):
            if self.essential_locations[lid] == "government":
                self.essential_locations[lid] = "dmv"
                print(
                    f"Marked {lid} with {unique[lid]} unique and {visits[lid]} overall visits as DMV location"
                )
                return
    ...
    def find_fraction_of_essential_visits_by_activity_type(self):
        ...
        visitors = defaultdict(
            lambda: defaultdict(
                lambda: {"essential": 0, "nonessential": 0, "dmv": 0, "residential": 0}
            )
        )
        for f in self.synth_pop_files["activities"]:
            with open(f, "r") as activities_in:
                ...
                for line in activities_in:
                    ...
                    des = (
                        self.essential_locations[activity[lid]]
                        if activity[lid] in self.essential_locations
                        else "nonessential"
                    )
        ...
        for day, visits in visitors.items():
            ...
            print(
                "\t",
                cancelling / total * 100 if total > 0 else 0,
                "% of OTHER activities will be cancelled",
            )
    ...
    def find_intersection_between_calculated_and_listed_location_designations(self):
        ...
        residentials = [
            x
            for x in self.essential_locations
            if self.essential_locations[x] == "residential"
        ]
        non_residentials = [
            x for x in self.essential_locations if x not in residentials
        ]

        essentials_not_in_pop = [
            x for x in non_residentials if x not in listed_location_assignments
        ]
        pop_not_in_essentials = [
            x for x in listed_location_assignments if x not in non_residentials
        ]

        pop_also_as_residential = [
            x for x in listed_location_assignments if x in residentials
        ]

        print(
            f"{len(essentials_not_in_pop)} of {len(non_residentials)} were NOT in activity schedules"
        )
        print(
            f"{len(pop_not_in_essentials)} of {len(listed_location_assignments)} listed were not in essential designation"
        )
        print(
            f"{len(pop_also_as_residential)} of {len(listed_location_assignments)} were in pop, but are also residential"
        )

        for x in non_residentials:
            if (
                x in listed_location_assignments
                and self.essential_locations[x] != listed_location_assignments[x]
            ):
                print(
                    x,
                    self.essential_locations[x],
                    listed_location_assignments[x]
                    if x in listed_location_assignments
                    else "",
                )
    ...
    def create_distribution_of_designation_bar_chart(self):
        ...
        labels = list(
            set(
                list(worker_location_distribution.keys())
                + list(located_location_distribution.keys())
            )
        )
        worker = [
            worker_location_distribution[x] if x in worker_location_distribution else 0
            for x in labels
        ]
        located = [
            located_location_distribution[x]
            if x in located_location_distribution
            else 0
            for x in labels
        ]
        set_d = [
            set_difference_distribution[x] if x in set_difference_distribution else 0
            for x in labels
        ]
    ...
    def __plot_ranges(labels, r1, r2, r3, pct):
        ...
        rects3 = ax.bar(
            x + width, r3, width, label="Located Activities (without overlap)"
        )

        ax.set_ylabel("Method of assigning designation")
        ax.set_xlabel(
            ("Percentage" if pct else "Number") + " of locations with designation"
        )
        ax.set_xticks(x)
    ...
    def plot_location_visitors(self):
        ...
        for loc, num in unique.items():
            ...
            designation = (
                self.essential_locations[loc]
                if loc in self.essential_locations
                else "none"
            )
        ...
        keys = list(
            set(
                [
                    item
                    for sublist in [
                        per_num_visits[y].keys() for y in per_num_visits.keys()
                    ]
                    for item in sublist
                ]
            )
        ...
        values = [
            [
                per_num_visits[n][label]
                if n in per_num_visits and label in per_num_visits[n]
                else 0
                for n in x
            ]
            for label in keys
