...
class Epicurve(object):
        ...
        with open(self.epicurve_file, "r") as f:
            for line in f.read().splitlines()[1:]:
                ...
                results.append(
                    dict(
                        zip(
                            self.epicurve_headers,
                            map(lambda x: int(x), line.split(split_char)),
                        )
                    )
                )
    ...
    def average_r_zero(self):
        ...
        return (
            len(r_0_relevant) / len(r_0),
            sum(r_0_relevant) / len(r_0_relevant) if len(r_0_relevant) > 0 else 0,
        )
    ...
    def get_score(self):
        ...
        infection_num_variation, max_infected_score = self.score_infected(
            self.get_pcts_infected()
        )
...
if __name__ == "__main__":
    ...
    print(
        "{0} agents in population, {1} days in the epicurve".format(
            epicurve.get_population_size(), len(epicurve.epicurve)
        )
    )
    print(
        "R-zero is {0} on average. Targetting an average r-zero of {1} this yields a score of {3} ({2} of all r-0 values were relevant)".format(
            r0avg, 1.5, r0avg_relevant, Epicurve.score_average_r(r0avg, 1.5)
        )
    )
    print(
        "No more infected agents after {0} days".format(
            epicurve.days_before_herd_immunity()
        )
    )
    print(
        "Infection score is {0} for the standard deviation and {1} for the maximum number of infected agents".format(
            *Epicurve.score_infected(pct_infected)
        )
    )
