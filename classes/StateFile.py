...
class StateFile(object):
	...
	def __init__(self, counties, frac_infected: float = 0, frac_symptomatic: float = .6):
	...
	def generate_from_fractions(self) -> str:
		...
		if not os.path.exists(self.__state_file):
			...
			for config in self.counties.values():
				self.__create_state_file(*self.__infect_random(self.__read_persons(config)))
	...
	def merge_from_config(self) -> str:
		...
		for c in self.counties:
			if 'statefile' in self.counties[c]:
				...
				healthy, symptomatic, asymptomatic = self.__infect_random(self.__read_persons(self.counties[c]))
				counties[c] = dict(
					healthy=healthy,
		...
		self.__state_file = "start_state_counties_" + self.__get_fips_list_string() + "_" + str(time.time()) + ".csv"
	...
	def __load_state_file(state_file_location: str) -> Dict[str, List[str] or float]:
		...
		return dict(
			...
			symptomatic=symptomatic,
			asymptomatic=asymptomatic,
			frac_infected=(len(symptomatic) + len(asymptomatic)) / (len(healthy) + len(symptomatic) + len(asymptomatic)),
	...
	def __infect_random(self, persons):
		...
		assert set(healthy).union(set(asymptomatic).union(set(symptomatic))) == set(persons)
	...
	def __create_state_file(self, healthy: List[str], symptomatic: List[str], asymptomatic: List[str]):
